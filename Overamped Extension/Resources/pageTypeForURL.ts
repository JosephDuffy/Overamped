export default function pageTypeForURL(url: URL): PageType {
  const pageHostname = url.hostname
  const currentGoogleDomain = googleDomains.find((googleDomain) => {
    return (
      pageHostname === googleDomain || pageHostname.endsWith(`.${googleDomain}`)
    )
  })
  if (currentGoogleDomain !== undefined) {
    // This is a Google domain
...
/// A list of all of Google's supported domains. These can be found from https://www.google.com/supported_domains.
